...
def Extract_HubFace_Points(x, y, z):
    ...
    threshold = 1500/100  # Define a threshold for detecting significant changes
    ...
    threshold = 500/100  # Define a threshold for detecting significant changes
...
def Calc_Plane(hubFacePoints):
    ...
    # Generate points on the best-fitting plane
    # Define a grid for the plane
    xx, yy = np.meshgrid(np.linspace(np.min(hubFacePoints[0]), np.max(hubFacePoints[0]), 100),
                        np.linspace(np.min(hubFacePoints[1]), np.max(hubFacePoints[1]), 100))
...
if __name__ == "__main__":
    data = np.loadtxt('measuredData7.txt', skiprows=1) #GenData(0.005)
    x = data[:,0]
    y = data[:,1]
    z = data[:,2]/100
    # Plot_All(x,y,z)
    hubFacePoints = Extract_HubFace_Points(x, y, z)
    # plane, xAngle, yAngle = Calc_Plane([x,y,z])

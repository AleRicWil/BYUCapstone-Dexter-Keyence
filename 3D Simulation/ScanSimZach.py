...
sys.path.insert(1, '../axle-components/')
import compileAxleSTLs as stl_fab
...
#    filename = r'STLs\Flatplate.STL'
#    myMesh = pv.read(filename)
#    myMesh.translate([-myMesh.center[0], -myMesh.center[1], -myMesh.center[2]])
#    myMesh = myMesh.rotate_x(0).rotate_y(0).rotate_z(0)  # 180, 0, 90 for axle stl

    myMesh = stl_fab.generate_full_mesh('../axle-components/', 'SimplifiedHub.STL', 'Bracket.STL', 'Shaft.STL', 'CrankArm7.STL', 
    ...
    alphas = np.array([0, 20*np.pi/180, -20*np.pi/180])
    print(alphas)
    ...
    with ProcessPoolExecutor() as executor:
        for i, alpha in enumerate(alphas):
            ...
            futures = [executor.submit(Scan_Profile_Sweep, myMesh, myMeshBounds, numPoints=numPoints, alpha=alpha, beta=beta, render=False, noise=False) for beta in betas]
            ...
            print(f"\rFinished scan {i}", flush=True)
    ...
    for i in range(len(myScans)):
        ...
        #Save_Scan(myScans[i][0], f'simScan{i}.txt')
